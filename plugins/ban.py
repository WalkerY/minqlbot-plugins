...
class ban(minqlbot.Plugin):
    ...
    def handle_game_start(self, game):
        ...
        teams = self.teams()
        self.players_start = teams["red"] + teams["blue"]
        countr = len(teams["red"])
        countb = len(teams["blue"])
        count = countr + countb
        if (count >= 6 and countr == countb):
            self.teamsize(countr)
            self.msg("^7Teamsize was automatically set to ^6{}^7.".format(countr))
            
        #auto-balance
        balance = self.plugins["balance"]
        if count % 2 == 0:
            balance.average_balance(minqlbot.chat_channel, self.game().short_type)
    ...
    def get_profile_thread(self, name, days):
        try:
            ...
            if not pro.is_eligible(days):
                ...
                # if there are no ppl on serv dont kick
                teams = self.teams()
