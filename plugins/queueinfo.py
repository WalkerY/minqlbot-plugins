...
class queueinfo(minqlbot.Plugin):
    ...
    def update_players_to_remove_list(self):
        ...
        plugins = ["ban", "balance"]
        # We check ban plugin then balance plugin
        # TODO Make it thread-safe?
        # self.plugins might be not thread-safe
        for plugin_ in plugins:
            if plugin_ in self.plugins:
                plugin = self.plugins[plugin_]
                if hasattr(plugin, "ban_flagged_lock"):
                    with plugin.ban_flagged_lock:
                        if hasattr(plugin, "ban_flagged"):
                            for player in plugin.ban_flagged:
                                name = player.clean_name.lower()
                                if name not in players_to_remove:
                                    players_to_remove[name] = player
                        else:
                            self.debug(
